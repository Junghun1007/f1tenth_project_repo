...
def generate_launch_description():
    ...
    lane_parameters = [
        ...
        ("lane_reconstruction_maximum_x_m", "2.30", float),
        ("lane_maximum_extrapolation_m", "0.50", float),
        ("lane_expected_width_m", "0.625", float),
        ("lane_width_tolerance_m", "0.20", float),
        ("lane_initial_center_tolerance_m", "0.30", float),
        ("lane_single_initial_tolerance_m", "0.20", float),
        ("lane_maximum_lateral_step_m", "0.08", float),
        ("lane_maximum_tracking_gap_m", "0.20", float),
        ("lane_minimum_points", "15", int),
        ("lane_maximum_fit_residual_m", "0.06", float),
        ("lane_allow_single_lane", "true", bool),
        ("lane_output_line_thickness_m", "0.04", float),
        ("lane_temporal_smoothing_alpha", "1.0", float),
        ("lane_maximum_temporal_jump_m", "0.20", float),
    ]
