...
자율주행 중 프리뷰 창에 포커스를 두고 `Space`를 누르면
`preview_stop_topic`(기본 `/auto/enabled`)으로 `false`를 발행한다.
`auto_control`은 수신 콜백에서 즉시 duty 0과 중앙 서보를 발행하며 정지 상태를
유지한다. 다시 출발하려면 `/auto/enabled`에 `true`를 명시적으로 발행해야 한다.
GUI 프리뷰를 끈 실행에서는 OpenCV 키 입력을 받을 수 없으므로 동작하지 않는다.
...
`lane_centerline_temporal_current_weight:=0.60`는 양쪽 중점의 현재 프레임을
60% 반영해 작은 프레임 지터를 줄인다. 양쪽에서 한쪽으로 바뀌는 첫 프레임은
