...
현재 프레임에서 실패하면 이전 평면으로 대신 판정하지 않습니다. 픽셀 히스테리시스와 군집 이력도
즉시 지우고 `ground_valid=false`, `INVALID | 이유`, 빈 장애물 목록을 출력합니다.
유효한 다음 프레임부터 다시 추정·확인합니다.
...
`ground.reset_history_height_m`(3cm)보다 많이 달라지면 검출 이력을 비우고 새 유효 평면으로
다시 확인합니다. 이 조건은 새 평면을 거부하는 조건이 아닙니다. 평면을 시간 평균하거나 이전
평면을 대체 사용하지 않습니다. 탐지/추정 설정 변경·카메라 재시작·프레임 공백에도 이력을 비웁니다.
...
유지군집의 `point_count`도 마지막 관측값입니다. 경과 시간은 프레임 시각 기준이며 전송 지연 전체는
포함하지 않습니다. 프레임 중단 시 만료 갱신에는 현재 단조 시각을 사용합니다.
