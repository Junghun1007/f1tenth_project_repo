...
바닥 평면 방식에서 돌아온 경우 `DLPLANE2` 파일은 읽을 수 없습니다. **B로 픽셀별 바닥 기준을
다시 측정**합니다. 이전 `DLFLOOR1` 파일이 그대로 남아 있고 카메라 설정이 같으면 재사용합니다.
기본 `frame_id`도 카메라 기준 `depth_lidar`로 복원되었습니다.
