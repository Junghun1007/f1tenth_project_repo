...
기존 주행을 허용한다. 시작 후 유효한 RED/GREEN을 한 번도 받지 않았다면 신호등
정지 조건을 적용하지 않는다. 한 번 인식한 뒤 UNKNOWN/입력 유실이 지속되면 마지막
유효 색상의 촬영 시각에서 `traffic_state_timeout_sec` 후 정지 요구를 적용한다. GREEN
유실은 임시 목표 속도 0으로 정지하며, 유효 GREEN이 돌아오면 해제한다. RED 유실은
기존 정지선 접근 요구를 유지한다. `signal_seen=no` 로그에서 정지한다면 신호등이
아닌 차선/ERPM/enable 조건을 확인한다. 정지선 거리 미검출은 `traffic_stop_line_timeout_sec`까지만
