...
회피 입력은 기존 중앙선 생성 결과를 그대로 사용한다. 한쪽 차선만 보여도 중앙선이
생성되면 계획할 수 있으며, 회피 모듈이 양쪽 관측을 별도로 필수 요구하지 않는다.
`obstacles.avoidance.centerline_fallback_enabled: true`(기본값)이면 경계가 없는 위치는
중앙선 생성기의 `centerline_lane_width_m`로 추정한다. BEV 화면 폭을 차선 폭으로 쓰지 않는다.
관측된 경계와 횡방향 차선 조각은 계속 차체 충돌 검사에 포함한다. 추정 경계는 실제
주행 가능 공간을 관측한 결과가 아니므로 차선 폭 설정/중앙선 오류가 회피에 영향을 준다.
옵션을 false로 두면 양쪽 경계가 확인되는 가까운 연속 구간까지만 사용한다.

중앙선의 1mm 이하 중복점을 정리하고, 원래 점의 연결 순서를 유지한다. 먼 곳의
역행/거의 횡방향인 코너, 곡률 한계를 넘는 지점, 20cm 초과 단절, 비정상 좌표 또는 BEV 밖 지점에서는
그 앞까지의 연속 경로를 사용한다. X 정렬로 분리된 구간을 연결하지 않는다.
경로를 호 길이 간격으로 다시 샘플링해 코너에서 인접 점 간격이 커지는 것도 방지한다.
실제 제어에서 시작점은 전방 25cm 이내여야 하며 유효 구간이 최소 50cm보다 짧거나
차량 크기/조향/장애물 조건을 만족하지 못하면 계속 정지한다.

속도 상한에는 사용 가능한 전방 길이에서 차체·여유 거리를 뺀 정지 공간도 반영한다.
`v*T + v²/(2*a)`가 이 공간 이내가 되도록 목표 속도를 제한하며,
`a=obstacles.avoidance.deceleration_mps2`, `T=obstacles.avoidance.stop_response_sec`이다.
응답 여유 기본값은 0.35초(허용 0.20..2.0초)이며 영상 나이와 구동기 지연을 포함하도록
설정한다. 감속도는 실측 제동 성능을 대신하는 보장이 아닌 계획 가정이다.
중앙선 입력 자체가 없거나 Depth가 만료되면 추정 경계로 이를 우회하지 않는다.
...
| `centerline_fallback_enabled` | true | 누락된 경계를 기존 중앙선과 차선 폭으로 추정. false면 양쪽 경계가 보이는 연속 구간만 사용 |
...
| `deceleration_mps2` | 0.5 | 유효 경로 끝까지의 정지 공간으로 속도를 제한하는 계획 감속도. 화면 전용에서는 장애물 접근에도 사용 |
| `stop_response_sec` | 0.35 | 경로 끝 정지 계산의 영상/제어/제동 응답 여유, 허용 0.20..2.0초 |
...
상태의 `C`는 중앙선 기반 추정 경계를 사용함, `P`는 가까운 연속 구간으로 경로를 잘랐음을
뜻한다. `L`은 사용한 중앙선 길이(m), `v`는 목표 속도 상한(m/s)이다.
